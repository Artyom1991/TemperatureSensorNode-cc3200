...
//#define POST_DATA           "{\n\"name\":\"xyz\",\n\"address\":\n{\n\"plot#\":12,\n\"street\":\"abc\",\n\"city\":\"ijk\"\n},\n\"age\":30\n}"
#define POST_DATA           "{\"name\":\"AlphaSensor\",\"date\":0,\"temperature\":20}"

#define HOST_NAME       	"192.168.0.100"//"<host name>"
...
#define POST_HTTP_INTERVAL_MS	10000UL
...
volatile unsigned char g_HTTP_SEND_FLAG;
...
static int HTTPPostMethod(HTTPCli_Handle httpClient) {
	...
	HTTPCli_FIELD_NAME_CONTENT_TYPE, "application/json" }, { NULL, NULL } };
	...
	/* Send POST data/body */
	lRetVal = HTTPCli_sendRequestBody(httpClient, POST_DATA,
			(sizeof(POST_DATA) - 1));
	if (lRetVal < 0) {
...
//*****************************************************************************
//
//! Timer0 and green LED initialization
//!
//! \param  None
//!
//! \return none
...
void configureTimerA0(void) {
	...
	//
	Timer_IF_Start(g_ulBase, TIMER_A, POST_HTTP_INTERVAL_MS);
}
...
int main() {
	...
	//
	//init timerA0 and green led
	//perform sending http data by timer interrupt
	//
	configureTimerA0();

	long postPerformRes = -1;
	//g_ulTimerInts++;
	...
	while (1) {
		if (g_HTTP_SEND_FLAG) {
			...
			//connect to server
			postPerformRes = ConnectToHTTPServer(&httpClient);
			if (postPerformRes < 0) {
				LOOP_FOREVER()
				;
			}
			...
			postPerformRes = HTTPPostMethod(&httpClient);
			if (postPerformRes < 0) {
				UART_PRINT("HTTP Post failed, result:%l.\n\r", postPerformRes);
			}
